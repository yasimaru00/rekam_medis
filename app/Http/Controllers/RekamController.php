    ...
    public function index(Request $request)
    {
        ...
        $rekams = Rekam::latest()
            ...
            ->leftJoin('dokter', function ($join) {
                $join->on('rekam.dokter_id', '=', 'dokter.id');
            })
            ...
            ->when($role, function ($query) use ($role, $user) {
                if ($role == "Dokter") {
                    $dokter = Dokter::where('id', $user->id)->where('status', 1)->first();
                    if ($dokter) {
                        $dokterId = $dokter->id;
            ...
            ->when($request->tab, function ($query) use ($request) {
                if (auth()->user()->role_display() == "Dokter" && $request->tab == 5) {
                    $query->whereIn('rekam.status', [3, 4, 5]); // Specify the table for the status column
                } else {
                    if ($request->tab == 5) {
                        $query->whereIn('rekam.status', [4, 5]); // Specify the table for the status column
                    } else {
                        $query->where('rekam.status', '=', $request->tab); // Specify the table for the status column
                    }
                }
            })
            ->paginate(10);
